...
pacc: float = 0.98  # threshold of predict accuracy
prec: float
# print(lc, hc)
...
sm = SMOTE(sampling_strategy=0.25, k_neighbors=10)
...
def adjust_window():
    ...
    if prec != -1:
        if recall > prec:
            if accuracy < pacc and recall > 0.9:
                metric = 2
            else:
                metric = 1
        else:
    ...
    if ratio < lc or metric == 0:  # and (len(PDES.items) != 0):
        ...
        return
    elif (ratio > (2 * hc) and metric == 1) or window_size > DEFAULT_MAX_WINDOW_SIZE:
        window_size -= 2
        i: int = 0
        pointer: int = 0
        while i < 2:
